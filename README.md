...
## v2 features (post-launch brainstorming)

Four creative updates + one structural rebuild landed on the same Stripe/Supabase wiring:

1. **Prime real-estate tiers.** Five legendary coords (69,69 / 69,42 / 04,20 / 42,42 / 50,50) cost 1.5× = $4.50. The four corners cost 1.25×. The center cross (Manhattan distance < 10 from the middle) costs 1.10×. Tier class glows + tier badge inside the claim modal.
2. **Audio chips.** Eight hardcoded 8-bit tones (Mario coin, Zelda secret, Tetris clear, Pacman dot, Windows XP ding, Doom pickup, Coin collect, 8-bit level up) generated client-side via Web Audio API. Plays on click +r fossilize moment. +$1.00 surcharge.
3. **Fossilization animation.** A 600ms `cubic-bezier` keyframe fires on every fresh commit (the buyer's screen + every Realtime-connected visitor's screen). Honors `prefers-reduced-motion`.
4. **Bury a Secret.** Passphrase-gated tiles. Server hashes via scrypt (built-in `node:crypto`, no npm bcrypt dep). Unlock endpoint compares with `timingSafeEqual`. Plaintext never stored.
5. **Faction Wars (v1 scaffolding).** Email magic-link sign-in via Supabase Auth; faction-create endpoint bootstraps the leader row. Schema + atomic block-reservation INSERT are complete; the rich block-reservation UI is v2. Individual tile purchases remain anonymous.

Documentation: [`STRIPE.md`](./STRIPE.md) walks through the multi-line itemization; [`supabase/README.md`](./supabase/README.md) covers migrations 0001–0005 + the new RLS posture.
